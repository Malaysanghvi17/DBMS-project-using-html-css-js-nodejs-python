...
def open_main_window():
    ...
    def create_tab(tab_name):
        ...
        if tab_name == "welcome tab":
            ...
            table_label2 = tk.Label(tab, text="select from above tabs to see all tables of database", font=("Arial", 20))
            table_label1.pack()
            table_label2.pack()
            ...
            mycursor.execute(f"SELECT si.name, si.roll, si.email, si.contact, si.address, si.birthdate, si.gender, c.course, c.stream, sc.semester, sc.cgpa, sh.roll, sh.hobby, sh.about FROM student_info si JOIN student_course sc ON si.roll = sc.roll JOIN course c ON sc.courseid = c.courseid LEFT JOIN student_hobbies_about sh ON si.roll = sh.roll;")
            ...
            label1 = tk.Label(tab, text=f"The values in this(joined) table is same as original table(form data table) hence it is lossless decomposition", font=("Arial", 15))
            label1.pack(pady=10)
            ...
            tab, text="select the row to be deleted from above table and press the corresponding buttons below:", font=("Arial", 15))
            table_label5.pack()
            ...
            search_button = tk.Button(search_frame, text="Search Query",
                             command=lambda: search_query(tab_name.replace(' ', '_'), treeview, search_textfield))
            ...
            # vsb = ttk.Scrollbar(tab, orient=tk.VERTICAL, command=treeview.yview)
            # # vsb.place()
            # vsb.pack(side="right", fill="y")
            # treeview.configure(yscroll=vsb.set)

            # hsb = ttk.Scrollbar(tab, orient=tk.HORIZONTAL, command=treeview.xview)
            # # hsb.place() 
            # hsb.pack()
            # treeview.configure(xscroll=hsb.set)
            ...
            if tab_name != "Course":
                ...
                table_label5 = tk.Label(
                    tab, text="select the row to be updated/deleted from above table and press the corresponding buttons below:", font=("Arial", 15))
                table_label5.pack()
